...
import '../../core/service/service_allocator.dart';
import '../domain/entities/genres.dart';


class MovieDetailTrailScreen extends StatelessWidget {
  final int id;

  const MovieDetailTrailScreen({Key? key, required this.id}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
  create: (context) => sl<MovieDetailsBloc>()..add(GetMovieDetailEvent(id))..add(GetMovieRecommendationsEvent(id)),
  ...
  builder: (context, state) {
    return SliverGrid(
      delegate: SliverChildBuilderDelegate(
        (context, index) {
          final recommendations = state.recommendations[index];
          return FadeInUp(
            from: 20,
            duration: const Duration(milliseconds: 500),
            child: ClipRRect(
              ...
              child: CachedNetworkImage(
                imageUrl: ApiConstants.imageUrl(recommendations.backDropPath!),
